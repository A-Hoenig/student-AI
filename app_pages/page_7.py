...
def page_7_body():
    ...
    with col1:
        ...
        caption = (
            f"Most significant features for predicting {selected_exam}"
        )
        st.image(image_path, caption=caption, width=250)

    with col2:
        train_report_path = f'confusion-matrix-{selected_exam}-train.txt'
        train_report = load_text(train_report_path)
        if train_report:
            text_size = "10px"  # Force HTML/CSS to set font
            st.markdown(
                f"<p style='font-size: {text_size};'>{train_report}</p>",
                unsafe_allow_html=True
            )

        test_report_path = f'confusion-matrix-{selected_exam}-test.txt'

        test_report = load_text(test_report_path)
        if test_report:
            st.markdown(
                f"<p style='font-size: {text_size};'>{test_report}</p>",
                unsafe_allow_html=True
            )
    ...
    st.info(
        ...
        * A false negative is problematic, as this represents students who
        **DO** need help that are predicted to not need help. This would go
        against the business objective, so the model performance and accuracy
        should avoid too many false negatives in this case.
